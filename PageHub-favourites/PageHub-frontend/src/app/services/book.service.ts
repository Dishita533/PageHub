...
addtoFavorite(book: Favorite) {
  ...
  const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
   return this.http.post(`http://localhost:5022/AddFavorites/`,book);
  //  return this.http.post(this.apiUrl1,book,{headers});
}
removeFromFavorite(bookId: number) {
  return this.http.delete(`api/favorites/${bookId}`);
}
